    ...
    public function get_items($request)
    {
        ...
        if (!empty($schedule_term)) {
            foreach ($schedule_term as $term) {
                ...
                $hadith = $this->list_posts_by_term(DIFeed_Post_Types::$HADITH, $term, 1);
                if (empty($hadith)) {

                    // get random one in case no spicific hadith in this day.
                    $hadith = $this->get_random_post(DIFeed_Post_Types::$HADITH, $options[DIFeed_Admin_Options::$HADITH['per_page']]);
                }

                // get ayah under term.
                $ayah = $this->list_posts_by_term(DIFeed_Post_Types::$AYAH, $term, 1);
                if (empty($ayah)) {

                    // get random one in case no spicific ayah in this day.
                    $ayah = $this->get_random_post(DIFeed_Post_Types::$AYAH, $options[DIFeed_Admin_Options::$AYAH['per_page']]);
                }
                ...
                if (empty($post)) {
                    ...
                    $post = $this->get_random_post('post', $options[DIFeed_Admin_Options::$POST['per_page']]);
                }

                // get ayah under term.
                $name = $this->list_posts_by_term(DIFeed_Post_Types::$NAMES, $term, 1);
                if (empty($name)) {

                    // get random one in case no spicific name in this day.
                    $name = $this->get_random_post('post', $options[DIFeed_Admin_Options::$POST['per_page']]);
                }
            }
        } else {
            $ayah = $this->get_random_post(DIFeed_Post_Types::$AYAH, $options[DIFeed_Admin_Options::$AYAH['per_page']]);
            $hadith = $this->get_random_post(DIFeed_Post_Types::$HADITH, $options[DIFeed_Admin_Options::$HADITH['per_page']]);
            $name = $this->get_random_post(DIFeed_Post_Types::$NAMES, $options[DIFeed_Admin_Options::$NAMES['per_page']]);
            $post = $this->get_random_post('post', $options[DIFeed_Admin_Options::$POST['per_page']]);
        }
